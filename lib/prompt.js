...
export function memoryPrompt(config) {
	return [
		...
		"When enabled, memory_plan create returns TypeSafe preflight coaching before work: resolve meaningful policy conflicts/gaps or record justified limitations, not ritual gates. A disabled, stale, uncertain or unavailable assessment is not compliance. Use memory_rsi preflight to refresh; scoped acceptance changes need an explicit successor plan. Earn achievements from outcomes and subsequent evidence review, never from model scores.",
		"Keep one coherent canonical policy; put job-specific guidance in templates/contracts and facts in memory. Use memory_rsi observe/reflect → corpus/mine/reduce for focused source-linked issues, then prepare → sections/propose → evaluate → promote (preview first). Prefer replacing/merging/retiring sections over appending rules; size is descriptive, not a target. Route tool defects to tool fixes. Jev judges typed evidence; the reasoning agent drafts. Repeated copies and model summaries are not independent evidence. Retain baseline when support is weak; examine counterexamples and later outcomes. Never silently weaken requirements, rewrite active plans, or claim improvement from assessor scores.",
