/**
 * memory-contract-rsi — a DeepSeek Harness (DSH) plugin package.
 *
...
export const name = "memory-contract-rsi";
 ...
 * @param {{cwd?: string, stdin?: string, timeoutMs: number, signal?: AbortSignal}} opts
...
export function apply(ctx, rawConfig) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (args.no_git) argv.push("--no-git");
			if (args.allow_non_main_branch) argv.push("--allow-non-main-branch");
			const stdin = args.body !== undefined ? (args.body === "" ? "-" : args.body) : undefined;
			if (stdin !== undefined) {
				argv.push("--body", "-");
				return { result: clip(await memory(config, argv, { signal: exec.signal, stdin })) };
			}
			return { result: clip(await memory(config, argv, { signal: exec.signal })) };
		},
	}));
