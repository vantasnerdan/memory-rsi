...
import { registerRsi } from "./rsi.js";
import { createSessionSignals } from "./rsi-signals.js";
import { memoryPrompt, resolveMemoryConfig } from "./prompt.js";
import { registerSetup, memoryEnvironment } from "./setup.js";
import { registerGitNexus } from "./gitnexus.js";
...
export const Config = z.object({
	...
	// Operator-owned allowlist; model calls cannot add arbitrary instruction paths.
	instructionFiles: z.array(z.string()).default([]),
	runtimeDir: z.string().default(""),
	bootstrapPython: z.string().default("python3"),
	gitnexusBin: z.string().default("gitnexus"),
	gitnexusHome: z.string().default(""),
	gitnexusTimeoutMs: z.number().default(120_000),
	setupTimeoutMs: z.number().default(600_000),
	timeoutMs: z.number().default(60_000),
	// Explicit consent to send selected policy/contracts to a remote semantic assessor.
	typesafeEnabled: z.boolean().default(false),
	// Local metadata-only session signals; persistence/disclosure stay explicit.
	rsiTelemetryEnabled: z.boolean().default(false),
	rsiLearningTimeoutMs: z.number().default(300_000),
	typesafeEndpoint: z.string().default("https://api.typesafe.ai/v1/systemone"),
	typesafeModel: z.string().default("jev-latest"),
	typesafeApiKeyEnv: z.string().default("TYPESAFE_API_KEY"),
	typesafeTimeoutMs: z.number().default(20_000),
	typesafeMaxRequestBytes: z.number().default(131072),
	typesafeMaxResponseBytes: z.number().default(262144),
	typesafeRetries: z.number().default(1),
});
 ...
 */
async function runCli(memoryBin, args, opts) {
	try { return (await runProcess(memoryBin, args, opts)).stdout; }
	catch (error) {
...
async function memory(config, args, opts = {}) {
	...
	const runOpts = {
		...
		cwd: opts.cwd ?? (config.base && existsSync(config.base) ? config.base : undefined),
		stdin: opts.stdin,
	...
	try {
		return await runCli(config.memoryBin, fullArgs, runOpts);
	} catch (error) {
...
export function apply(ctx, rawConfig) {
	...
	const signals = createSessionSignals(ctx, config);
	const rsi = registerRsi(ctx, config, { memory, clip, output: cliOutput, signals });
	registerContracts(ctx, config, { memory, clip, output: cliOutput, rsi });
	ctx.systemPrompt.section({
		name: "tool:memory-rsi",
		order: 2150,
		text: () => memoryPrompt(config),
	});
}
